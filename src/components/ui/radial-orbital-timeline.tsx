...
import { ArrowRight, Link, Zap, Search, FileText, Code, TestTube, Rocket, LucideIcon } from "lucide-react";
...
export default function RadialOrbitalTimeline({
  ...
  const getStatusStyles = (status: TimelineItem["status"]): string => {
    switch (status) {
      ...
      case "pending":
        return "text-white bg-neutral-800 border-neutral-600";
      default:
        return "text-white bg-neutral-800 border-neutral-600";
    }
  };
  ...
  return (
    <section className={cn("w-full bg-black py-24 overflow-hidden", className)}>
      {/* Header */}
                ...
                >
                  ...
                  <div
                    className={cn(
                      ...
                      isExpanded
                        ? "bg-white text-black border-white shadow-lg shadow-purple-500/30 scale-150"
                        : isRelated
                        ? "bg-purple-500/50 text-white border-purple-400 animate-pulse"
                  ...
                  <div
                    className={cn(
                      "absolute top-12 whitespace-nowrap text-xs font-light tracking-wider transition-all duration-300",
                  ...
                  {isExpanded && (
                    <Card className="absolute top-20 left-1/2 -translate-x-1/2 w-72 bg-black/95 backdrop-blur-lg border-white/20 shadow-xl shadow-purple-500/10 overflow-visible">
                      ...
                      <CardHeader className="pb-2">
                        ...
                        <CardTitle className="text-base mt-2 font-light tracking-tight text-white">
                          {item.title}
                        </CardTitle>
                      </CardHeader>
                      <CardContent className="text-xs text-white/80 font-light">
                        <p>{item.content}</p>
                              ...
                              {item.relatedIds.map((relatedId) => {
                                ...
                                return (
                                  <Button
                                    ...
                                    variant="outline"
                                    size="sm"
                                    className="flex items-center h-6 px-2 py-0 text-xs rounded-none border-white/20 bg-transparent hover:bg-purple-500/20 text-white/70 hover:text-white transition-all"
                                    onClick={(e) => {
                                      e.stopPropagation();
